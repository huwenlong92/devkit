...
DK 是一组面向日常开发环境的 zsh 小工具，用来把常用命令、代理开关和 Codex 工作流收进一个可复用的 shell 工具箱。配置后只需要 source 一次入口脚本，就可以在任意终端里使用这些快捷命令。
...
- rustup Rust 版本管理：安装 `rustup` 后由 `shell/rust.zsh` 加载 Cargo 环境，并按项目里的 `rust-toolchain.toml` / `rust-toolchain` / `.rust-version` 自动锁定工具链，离开项目后回到默认工具链。
